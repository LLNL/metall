...
#else
constexpr std::size_t k_chunk_size = 1ULL << 20ULL;
#endif
...
#pragma omp parallel for schedule (runtime)
    ...
    std::cout << "elapsed_time(s)" << "\t" << elapsed_time << std::endl;
    std::cout << "mem_usage(gb)" << "\t" << static_cast<double>(utility::get_used_ram_size()) / (1ULL << 30) << std::endl;
    print_current_num_page_faults();
