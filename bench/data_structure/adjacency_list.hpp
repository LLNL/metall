...
class adjacency_list {
 ...
 private:
  ...
  /// see: https://stackoverflow.com/questions/36831496/unordered-map-with-string-in-managed-shared-memory-fails
  using key_table_allocator_type = bct::scoped_allocator_adaptor<other_allocator_type<std::pair<const key_type, list_type>>>;
  using key_table_type = boost::unordered_map<key_type, list_type, std::hash<key_type>, std::equal_to<key_type>,
 ...
 public:
  ...
  explicit adjacency_list(const _base_allocator_type &allocator = _base_allocator_type())
  ...
  key_iterator keys_begin() {
    return m_key_table.begin();
  }

  key_iterator keys_end() {
    return m_key_table.end();
  }
