...
struct bfs_data {
  ...
  static constexpr level_type k_infinite_level = std::numeric_limits<level_type>::max();
...
void initialize(const std::size_t max_vertex_id, const vertex_id_type &source, bfs_data *data) {
...
bfs_data::level_type kernel(const graph_type &graph, bfs_data *data) {
  ...
  while (true) { /// BFS main loop
    ...
    for (typename graph_type::key_type source = 0; source < num_vertices; ++source) {
      ...
      for (auto neighbor_itr = graph.values_begin(source), neighbor_end = graph.values_end(source);
           neighbor_itr != neighbor_end; ++neighbor_itr) {
        const auto &neighbor = *neighbor_itr;
        if (!data->visited_filter.at(neighbor) && data->level.at(neighbor) == bfs_data::k_infinite_level) {
...
void count_level(const bfs_data& data) {
  ...
  std::cout << "Total\t" << std::accumulate(cnt.begin(), cnt.end(), static_cast<std::size_t>(0)) << std::endl;
}
