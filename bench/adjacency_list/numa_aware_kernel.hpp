...
#pragma omp parallel
  {
    if (omp::get_thread_num() == 0) {
      ...
      std::cout << "#numa nodes\t" << bench_utility::numa::get_avail_nodes() << std::endl;
...
inline double numa_aware_kernel(const std::size_t chunk_size,
  ...
  while (true) {
    ...
    OMP_DIRECTIVE(parallel)
    {
      ...
      const auto local_thread_num = bench_utility::numa::get_local_thread_num(thread_id);
      const auto num_local_threads = bench_utility::numa::get_local_num_threads(thread_id, num_threads);
      const auto range = mdtl::partial_range(key_value_list.size(), local_thread_num, num_local_threads);

      for (std::size_t i = range.first; i < range.second; ++i) {
        // If the key is assigned to the local numa node, add to the adjacency list
        if (adj_list->partition_index(key_value_list[i].first) == bench_utility::numa::get_node(thread_id)) {
