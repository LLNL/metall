...
class pair_reader
{
 public:
  ...
  iterator begin()
  {
    return iterator(m_file_name_list);
  }
...
class pair_reader<first_type, second_type>::iterator
{
 ...
 private:
  ...
  void open_file()
  {
    ...
    if (!m_ifs.is_open()) {
      std::cerr << __FILE__ << ":" << __LINE__ << " Can not open " << m_current_file_name << std::endl;
      std::abort();
    }
  }
...
//template <typename first_type, typename second_type>
//inline bool operator==(const typename pair_reader<first_type, second_type>::iterator &rhd,
//                       const typename pair_reader<first_type, second_type>::iterator &lhd)
//{
