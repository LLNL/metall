...
/// \brief Simple CSR graph data structure that can take a custom C++ allocator and be stored in persistent memory
/// \tparam index_t Index type
...
class csr {
 private:
  ...
  using index_allocator_t = typename std::allocator_traits<allocator_t>::template rebind_alloc<index_t>;
  ...
  using edge_allocator_t = typename std::allocator_traits<allocator_t>::template rebind_alloc<vid_t>;
 ...
 public:
  ...
  index_t *indices() {
    ...
    // return std::pointer_traits<char_pointer_t>::to_address(m_indices); // From C++20
  }

  /// \brief Returns a raw pointer to the edges array
  vid_t *edges() {
    return metall::to_raw_pointer(m_edges);
    // return std::pointer_traits<char_pointer_t>::to_address(m_edges); // From C++20
  }
