...
// We recommend you to use containers implemented in Boost libraries because they fully support the offset pointer
...
// Inner multimap type
using multimap_type = unordered_multimap<uint64_t,
                                         vector_type,
                                         std::hash<uint64_t>,
...
using map_allocator_type = scoped_allocator_adaptor<metall::manager::allocator_type<std::pair<unsigned int, multimap_type>>>;
using map_type = unordered_map<unsigned int,
                               multimap_type,
...
int main() {

  {
    ...
    metall::manager manager(metall::create_only,
                            "/tmp/datastore/version_0"); // Prefix of the backing files

    // Construct an object of map_type
    map_type *pmap = manager.construct<map_type>
        ("map") // Name of the constructed object
        (manager.get_allocator<>()); // Call map_type's constructor with Metall allocator object
    ...
    // Take a snapshot
    manager.snapshot("/tmp/datastore/datastore_snapshot");
  }

  {
    // Open the snapshot
    metall::manager manager(metall::open_only, "/tmp/datastore/datastore_snapshot");
