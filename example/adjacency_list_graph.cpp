...
using adj_list_graph_t = adjacency_list<vid_t, metall::manager::allocator_type<char>>;
...
int main() {
...

  {
    ...
    metall::manager manager(metall::open_only, "/tmp/dir/version_0");

    adj_list_graph_t *adj_list_graph = manager.find<adj_list_graph_t>("adj_list_graph").first;

    // Add another edge
    adj_list_graph->add_edge(1, 3);

    // Print the edges of vertex 1
    for (auto edge = adj_list_graph->edges_begin(1); edge != adj_list_graph->edges_end(1); ++edge) {
