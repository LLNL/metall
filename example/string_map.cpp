...
void string_int_map() {
  using value_type = std::pair<const persistent_string, // Key type
                               int>; // Mapped type
  using string_int_map = boost::container::map<persistent_string,
                                               int,
...

  {
    ...
    std::cout << pmap->at(persistent_string("one", manager.get_allocator<>())) << std::endl; // Will print "1"
  }
}
...
void int_string_map() {
  ...
  // Scoped allocator is required to force to use user given allocators in inner containers
  using map_allocator_type = boost::container::scoped_allocator_adaptor<metall::manager::allocator_type<value_type>>;
  using int_string_map = boost::container::map<int,
                                               persistent_string,
                                               std::less<int>,
                                               map_allocator_type>;

  {
    ...
    // Mapped objects (persistent_string) use an allocator stored in pmap automatically thanks to scoped allocator
    (*pmap)[2].push_back('t');
    (*pmap)[2].push_back('w');
    (*pmap)[2].push_back('o');
  }
