...
int main() {
  {
    ...
    // Allocate an offset pointer with key 'ptr' and initialize its value with the address of 'array'
    [[maybe_unused]] int_offset_prt* ptr = manager.construct<int_offset_prt>("ptr")(array);
  }

  {
    ...
    // If a raw pointer is given, it just returns the address the given pointer points to
