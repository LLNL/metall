...
/// \tparam outer_iterator_type
/// \tparam inner_iterator_type
...
class container_of_containers_iterator_adaptor {
 public:
  ...
  using iterator_category = std::forward_iterator_tag; // TODO: expand to bidirectional_iterator_tag
 ...
 private:
  void next() {
    ...
    while (true) {
      m_outer_iterator = std::next(m_outer_iterator); // move to the next container
      if (m_outer_iterator == m_outer_end) {
