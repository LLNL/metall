...
inline int comm_rank(const MPI_Comm &comm) {
  ...
  if (::MPI_Comm_rank(comm, &rank) != MPI_SUCCESS) {
    logger::out(logger::level::error, __FILE__, __LINE__, "Failed MPI_Comm_rank");
    return -1;
  }
  return rank;
}
...
inline int comm_size(const MPI_Comm &comm) {
  ...
  if (::MPI_Comm_size(comm, &size) != MPI_SUCCESS) {
    logger::out(logger::level::error, __FILE__, __LINE__, "Failed MPI_Comm_size");
    return -1;
  }
  return size;
}
...
/// \return A pair of bools (error, global value). 'error' is set to false if there is an error.
inline std::pair<bool, bool> global_logical_and(const bool local_value, const MPI_Comm &comm) {
  char global_value = 0;
  if (::MPI_Allreduce(&local_value, &global_value, 1, MPI_CHAR, MPI_LAND, comm) != MPI_SUCCESS) {
...
/// \param comm MPI communicator.
/// \return A pair of bools (error, global value). 'error' is set to false if there is an error.
inline std::pair<bool, bool> global_logical_or(const bool local_value, const MPI_Comm &comm) {
  char global_value = 0;
  if (::MPI_Allreduce(&local_value, &global_value, 1, MPI_CHAR, MPI_LOR, comm) != MPI_SUCCESS) {
...
inline int determine_local_root(const MPI_Comm &comm) {
  ...
  if (shm_fd == -1) {
    ...
    if (shm_fd == -1) {
      logger::out(logger::level::error, __FILE__, __LINE__, "Failed to open & create a shm file");
      return -1;
    }
  ...
  if (world_rank < world_size - 1) {
    if (MPI_Send(nullptr, 0, MPI_BYTE, world_rank + 1, 1, MPI_COMM_WORLD) != MPI_SUCCESS) {
  ...
  if (this_rank_created && ::shm_unlink(shm_name) != 0) {
    logger::perror(logger::level::warning,
                   __FILE__,
                   __LINE__,
