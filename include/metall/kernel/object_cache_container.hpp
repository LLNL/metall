...
template <std::size_t k_num_bins, std::size_t k_bin_size,
          typename bin_no_manager>
...
template <std::size_t k_num_bins, std::size_t k_bin_size,
          typename bin_no_manager>
constexpr std::array<std::size_t, k_num_bins> capacity_table =
    init_capacity_table<k_num_bins, k_bin_size, bin_no_manager>();
...
inline constexpr auto init_offset_table() noexcept {
  ...
  for (std::size_t b = 0; b < offset_table.size() - 1; ++b) {
    offset_table[b + 1] =
        offset_table[b] +
        capacity_table<k_num_bins, k_bin_size, bin_no_manager>[b];
  }
  return offset_table;
}
...
template <std::size_t k_num_bins, std::size_t k_bin_size,
          typename bin_no_manager>
constexpr std::array<std::size_t, k_num_bins> offset_table =
    init_offset_table<k_num_bins, k_bin_size, bin_no_manager>();
...
template <std::size_t _k_bin_size, std::size_t _k_num_bins,
