 ...
 private:
  static path_type priv_get_root_path(const std::string& raw_path) {
    ...
    if ( stash_path_index != std::string::npos) {
      base_dir = raw_path.substr((stash_path_index + 7), raw_path.length() - (stash_path_index + 7));
    }
    else{
      base_dir = raw_path;
    }
...
class privateer_segment_storage {
 public:
  ...
  static std::size_t get_size(const std::string &base_path) {
    ...
    return Privateer::version_capacity(parse_path(base_path).first.c_str()); // TODO: Implement Static get_size in Privateer
  }
  ...
  bool create(const std::string &base_path,
              const std::size_t vm_region_size,
  ...
  /// \return Returns true on success; otherwise, false.
  bool open(const std::string &base_path, const std::size_t vm_region_size, void *const vm_region, const bool read_only) {
    assert(!priv_inited());
 ...
 private:
  ...
  bool priv_map_file_create(const std::string &path, const std::size_t file_size, void *const addr) {
    assert(!path.empty());
    assert(file_size > 0);
    assert(addr);
