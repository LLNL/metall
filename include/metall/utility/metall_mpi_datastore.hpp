...
/// The MPI ranks that share the same storage space create their data stores under the root directory.
/// \param root_dir_prefix A prefix of a root directory path.
/// \return A path of a root directory.
...
/// The path can be passed to, for example, metall::manager to perform operations.
inline std::string make_local_dir_path(const std::string &root_dir_prefix, const int rank) {
  return make_root_dir_path(root_dir_prefix) + "/subdir-" + std::to_string(rank);
}
