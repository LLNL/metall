...
/// This container does not allocate mutex objects internally, but allocates them as static objects.
/// To achieve high concurrency, this container allocates multiple banks, where a bank consists of
/// an actual STL map object and a mutex object.
/// \tparam _key_type A key type.
/// \tparam _mapped_type A mapped type.
/// \tparam _compare A key compare.
/// \tparam _bank_no_hasher A key hasher.
/// \tparam _allocator An allocator.
/// \tparam k_num_banks The number of banks to be allocated.
...
class concurrent_map {
 ...
 public:
  ...
  /// \param value An element value to insert.
  ...
  /// If no element exists with an equivalent key, this container creates a new element with key.
  /// \param key A key of the element to edit.
  /// \return A pair of a reference to the element and a mutex ownership wrapper.
  ...
  /// If no element exists with an equivalent key, this container creates a new element with key.
  /// \param key A key of the element to edit.
  /// \param editor A function object which edits an element.
